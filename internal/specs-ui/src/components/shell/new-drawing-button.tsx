...
/**
 * Botão "+" do header da seção Desenhos. Abre dialog com textarea e a mesma
 * escolha de CLI/modelo/esforço das outras seções. Ao submeter, dispara o agent
 * `drawing-agent`, que grava um `.md` com Mermaid em `.specs/drawings/`.
 */
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.mode === 'external') {
        toast.success('Agent `drawing-agent` iniciado em novo terminal.')
      } else {
        upsertJob(res.job)
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button
          type="button"
          aria-label="Novo desenho (abrir dialog do drawing-agent)"
          title="Novo desenho"
          className={cn(
        ...
        <Dialog.Content className="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl shadow-2xl focus:outline-none w-[540px] max-w-[calc(100vw-32px)]">
          ...
          >
            <div className="flex items-center gap-2">
              <PenLine className="size-[18px] text-[var(--accent)]" aria-hidden="true" />
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ padding: '16px 24px' }}>
              <Dialog.Description className="text-sm text-[var(--text-secondary)] mb-3">
                Descreva o que o desenho precisa mostrar. O agent{' '}
                <code className="text-[var(--accent)]">drawing-agent</code> lê o código do projeto,
                monta o diagrama em Mermaid e grava em{' '}
                <code className="text-[var(--accent)]">.specs/drawings/</code>.
              </Dialog.Description>
              <textarea
                autoFocus
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="Ex: arquitetura do fan-out do SNS domain-events para notification-q e audit-q, mostrando filter policy, DLQ e quem consome cada fila..."
                rows={6}
                required
                disabled={submitting}
                className={cn(
              ...
              <p className="text-xs text-[var(--text-muted)] mt-2">
                Dica: diga o recorte (que componentes entram), o público (quem vai ler) e a pergunta
            ...
            >
              ...
              <button
                ...
                style={{ height: '32px', padding: '0 14px' }}
              >
                <PenLine className="size-[13px]" aria-hidden="true" />
