...
/**
 * Dialog com editor CodeMirror 6 para o arquivo .md de um desenho, chamando
 * PUT /api/drawing/content — editar o Mermaid na mão é o caminho rápido quando
 * o ajuste é de uma linha e não vale acordar um agent.
 *
 * Atalhos: Cmd/Ctrl+S para salvar. Esc para fechar.
 */
export function EditarDrawingDialog({ open, onOpenChange, drawingSlug }: Props) {
  const qc = useQueryClient()
  const { data: content, isLoading } = useDrawingContent(drawingSlug)
  ...
  // Qual (dialog aberto + desenho) já foi semeado no editor. Com o watcher de
  ...
  useEffect(() => {
    ...
    if (seededFor.current === drawingSlug) return
    seededFor.current = drawingSlug
    setValue(content.raw)
    setDirty(false)
  }, [open, content?.raw, drawingSlug])

  const mutation = useMutation({
    mutationFn: () => api.updateDrawingContent(drawingSlug, value),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['drawing-content', drawingSlug] })
    ...
    onError: () => {
      toast.error('Não foi possível salvar o desenho. Tente novamente.')
    },
  })
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <Dialog.Description className="sr-only">
            Editor markdown do arquivo .md do desenho. Cmd+S salva. Esc fecha.
          </Dialog.Description>
