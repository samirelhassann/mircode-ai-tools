...
/**
 * Dialog com editor CodeMirror 6 para o arquivo .md de uma discovery. Reusa o
 * mesmo padrão do `EditarTaskDialog`, chamando PUT /api/discovery/content.
 *
 * Atalhos: Cmd/Ctrl+S para salvar. Esc para fechar.
 */
export function EditarDiscoveryDialog({ open, onOpenChange, discoverySlug }: Props) {
  const qc = useQueryClient()
  const { data: content, isLoading } = useDiscoveryContent(discoverySlug)
  ...
  // Qual (dialog aberto + discovery) já foi semeado no editor. Com o watcher de
  ...
  useEffect(() => {
    ...
    if (seededFor.current === discoverySlug) return
    seededFor.current = discoverySlug
    setValue(content.raw)
    setDirty(false)
  }, [open, content?.raw, discoverySlug])

  const mutation = useMutation({
    mutationFn: () => api.updateDiscoveryContent(discoverySlug, value),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['discovery-content', discoverySlug] })
      void qc.invalidateQueries({ queryKey: ['tree'] })
      toast.success('Discovery atualizada.')
      setDirty(false)
      onOpenChange(false)
    },
    onError: () => {
      toast.error('Não foi possível salvar a discovery. Tente novamente.')
    },
  })
      ...
      <Dialog.Portal>
        ...
        >
            ...
            <div className="flex items-center gap-2 min-w-0">
              <Dialog.Title className="text-[14px] font-bold text-[var(--text-primary)] truncate">
                ...
                <code className="text-[var(--accent)] font-mono text-[13px]">
                  .specs/discoveries/{discoverySlug}.md
                </code>
              </Dialog.Title>
              {dirty ? (
          ...
          <Dialog.Description className="sr-only">
            Editor markdown do arquivo .md da discovery. Cmd+S salva. Esc fecha.
          </Dialog.Description>
